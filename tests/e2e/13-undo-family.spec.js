/* ⚠️★[이 그물의 구멍 · 2026-09-22] 이 판의 «먹통 칸(U3)» 축은 못 믿는다.
 *
 * 기전: _undo-family-harness.js 의 unit() 이 끝에서 `.replace(/\s+/g, ' ')` 만 한다.
 *   태그 사이 공백이 «지워지지 않고 한 칸으로» 남아 `> <` 와 `><` 가 «다른 글자»가 된다.
 *   ⇒ 재직렬화에서 «공백만» 달라져도 지문이 「바뀌었다」로 세어진다.
 *   (다시 쓰는 쪽 wt-eval-undo 가 짚었고, unit() 을 읽어 기전을 확인했다.)
 *
 * ★이 결함은 «두 방향»으로 튄다 — 한 방향만 적으면 다음 사람이 반대쪽에서 속는다:
 *   · 무변화를 «못 본다»(거짓 초록) — U3 의 dead · U1 의 stuck · U5 의 `changed === 0`
 *   · 없는 변화를 «본다»(거짓 빨강) — U4 의 numUndoChanged===1 · U2 의 reachedStart/undosToStart
 * ⛔그러니 U3 초록을 「먹통 칸 없음」으로 읽지 마라. 「이 계측기로는 못 봤다」가 전부다.
 *
 * ★반대로 «순수 정수»로 재는 줄은 이 결함이 안 낀다 — 앱이 준 getHistoryTip().pos 의 차다:
 *   U1 의 lineEditSlot·editSlot(T-009 과 그 형제) · ladder 의 notes[].slot · T-135 의 numSlot.
 *   오늘 dev 로 나간 «되돌리기 전면 무동작»을 잡은 것도 그 정수 축이다.
 *
 * ⇒ 고친 판이 «다시 쓰이는 중»이다(축 ㉠/㉡ 합침 · 정규화 `>\s+<` → `><` · T-140 을 사다리에서 뺌).
 *   들어오면 ⛔부분 수정하지 말고 «이 파일을 통째로» 갈아라 — 축이 합쳐져서 줄이 안 맞는다.
 *   ★대조용 실측(이 판 · 2a6af5d · 2026-09-22 · E2E_UNDO_PORT=9538 격리 프로필 · retries=0):
 *     기준선 7초록 / 4빨강(U1×2 · U5×2)
 *     → row.id 고침만 되돌리면 6빨강, «늘어난 것이 정확히 U2 둘».
 *     새 판이 이 수와 안 맞으면 «다시 쓰다 재는 양이 바뀐 것»이다.
 *   ★U5 빨강 2건은 결함이 아니라 «계측 실패»다 — L4/③GIF로적용 = covered-by:null
 *     (#vtrim-apply-gif-btn 이 화면 밖이라 elementFromPoint 가 null 을 준다). 안 고치면
 *     L4 사다리(T-012 GIF 적용)는 영영 안 재진다.
 */
/* ══════════════════════════════════════════════════════════════════════════
