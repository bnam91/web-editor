 ...
 *    ★당시 근거(실측): main.js `_listProjectsImpl` 의 두 걸름망이 모두 /^proj_\d+$/ 라
 *      plan_* 프로젝트는 «만들어져도 목록에 안 뜬다»(9397 실기 재현: plan_1789885549682 가
 *      디스크에 생겼는데 projects:list 는 [] 를 돌려줬다). 버튼이 살아 있으면
 *      «만들었는데 사라지는» 경험이 된다.
 *    ★★2026-09-21 — 그 근거는 T-064 로 사라졌다(목록이 plan_<숫자> 도 읽는다 —
 *      tests/unit/list-plan-projects.test.js). 그런데도 이 검사는 «그대로» 남는다:
 *      단추를 숨겨 두는 것은 이제 현빈 지시다(2026-09-21 「되살리지 마라」).
 *      ⇒ 켜는 조건은 «기술»이 아니라 «현빈의 말»이다. 그 말이 있기 전엔 여기를 뒤집지 마라.
