 ...
 *        이 자가 지키던 뜻(바깥 `data-line` 의 소비자 ★아홉 — 명부는 grid-block.js `_gridNestAddr`
 *        위 주석 — 이 중첩 속을 안 집는다)은 그대로고,
 ...
 *    ⛔여기서도 지운 것은 «하나도» 없다. */
/*  ★2026-09-24 다섯째 판: 299 → 310. 더한 것 = tests/unit/grid-gap-clamp.test.js 의 11개
...
/* ★래칫 — 기존 grid 유닛 검사의 «수». ⛔하한이 아니라 등호다(G5 주석 참조).
   ...
   · ★★2026-09-27 (현빈 T-230 「마지막 한칸도 비울 수 있게 해줘») — 그 둘이 «뒤집혔다».
     막던 자를 함수째 지웠고, 같은 자리에 같은 수(3)를 다시 세웠다 — 그래서 래칫은 «그대로»다:
       「★범위 — 내용이 남은 «마지막 칸»도 비워진다 (블럭이 통째로 빈다)」
       「★범위 — «옆 칸에 내용이 있든 없든» 같게 통과한다 (판정이 옆 칸을 더는 안 본다)」
       「★음성대조 — 막던 code 이름이 소스에서 «사라졌는가» (되살아나면 빨개진다)」
     ★★래칫이 «수»만 보기 때문에 여기서 아무 일도 안 일어났다. ⛔그것이 이 자의 한계다 —
       계약이 통째로 뒤집혀도 수가 같으면 침묵한다. 그래서 «뒤집힘»은 이 주석에 남긴다.
   · ★335 (2026-09-27 ②, 현빈 T-184 「찌꺼기는 남으면 안되지」) — grid-cell-unset-contract.test.js 에 «+2».
     D1~D6 은 `patchCell` 한 입구만 쟀는데, 같은 일을 하는 «다른 입구»(cells 통째, MCP 가 오는 길)가
     `null` 을 «값으로» 저장하고 있었다. 그래서 더한 둘:
       「D7 ★★두 입구가 «같은 답»을 준다 — patchCell 과 cells 통째」(40쌍을 나란히 놓고 센다)
       「D8 ★음성대조 — `cells` 쪽 「지움」을 걷어내면 D7 이 빨개진다」
     ★D8 은 «patchCell 쪽은 멀쩡한지»도 같이 단언한다 — 변이가 둘을 같이 망가뜨렸다면
       그 빨강이 「cells 자리를 재고 있다」를 증명하지 못하기 때문이다.
   ★★이 래칫이 오늘 «값»을 했다 — 고치다가 `_gridCellsToDataset` 안에 같은 뜻의 for 문을 «새로»
     썼더니 지움 길이 둘이 됐고, D5(「그 두 줄이 유일한 지움 길이다」)가 바로 빨개져 잡았다.
     ⇒ ★같은 뜻의 코드를 두 번 쓰면 «따로 늙는다». 부르는 쪽을 늘리고 규칙은 한 곳에 둔다.
   · ★342 (2026-09-27 ③, T-220) — grid-line-add.test.mjs 에 «+7».
     중첩 «안» 줄로 가는 «쓰기» 길(patchCell{lineIndex, np})을 냈다. 일곱을 «갈라» 잰다:
       W1 중첩 안 줄이 고쳐지고 «옆 열·바깥 줄은 안 바뀐다»
       W2 둘째 열도 따로 고쳐진다(열을 실제로 가른다)
       W3 없는 주소는 «거절»한다(지어내지 않는다) ＋ 데이터 무변
       W4 `np` 는 `lineIndex` 없이는 거절(조용한 무시 금지)
       W5 꼴이 틀린 주소 여섯 꼴 ＋ «깊이 초과»도 거절(상한 값을 검사에 안 베낀다)
       W6 옛 길(np 없는 patchCell)은 한 글자도 안 바뀐다
       W7 ★음성대조 — 적용부의 중첩 분기를 떼면 W1 이 빨개진다
     ⛔되는 것만 두면 「언제나 통과한다」와 구별이 안 된다 — 그래서 안 되는 것 셋을 같이 세웠다. */
const GRID_BASELINE_TESTS = 342;
