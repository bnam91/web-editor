...
/* ⚠️★★같은 격자인데 «수가 다르게» 나오면 「누가 틀렸나」보다 «자가 다른가»를 먼저 물어라.
     실제로 그랬다(2026-09-09): 적대적 검수는 옛 규칙의 삐져나감을 55,296칸에서 23,136칸으로,
     나는 24,288칸으로 셌다 — 1,152칸 차이. ★최댓값은 228.68px 로 «정확히» 같았다.
     원인은 원(circle)을 재는 자다. 검수는 윤곽을 «점으로 훑고»(표본 사이로 얕은 삐짐이 샌다),
     위 overshoot 은 「반지름 − 중심~선 거리」로 «해석적»으로 푼다 — 얕은 것까지 잡는다.
     ⇒ 최댓값이 같고 «건수만» 갈리면 표본이 새는 쪽을 의심해라. 여기 자가 더 엄하다.
