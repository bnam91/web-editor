...
test('EN6c ★리스트 뷰 예약폭이 «버튼 개수»와 맞는다 — 아이콘을 뺐으면 폭도 뺀다', () => {
  ...
  const px = Number(m[1]);
  // ★2026-09-16(T-A) — 「폴더로 이동」(📁) 버튼이 상시로 추가돼 기본이 «4개»(즐겨찾기·폴더·복제·삭제)가
  //   됐다: 4×22 + 3×4 + 10 = 110 (공유까지 겹치는 5번째는 이 표의 기존 한계 밖, 이전부터 그랬다).
  assert.ok(px >= 106 && px <= 114,
    `★예약폭 ${px}px — 버튼 3개 시절(84px) 값이 남아 있으면 「폴더 이동」 아이콘이 .card-date 와 겹친다`);
});
