/* ★[H1 양성대조 고정자료, 2026-09-06] dev 9aac92b 시점의 main/report/queue.js 원문 «그대로»
 *   (git show dev:main/report/queue.js 로 「고치기 전」을 매번 새로 뜨면 이 저장소가 dev 를
 *   로컬 브랜치로 안 갖고 있는 클론·CI 에서 검사가 죽는다 — U-M67 스위트의 «옛 산식을 인라인
 *   으로 못 박는» 관례와 같은 이유로, 여기서는 파일째로 얼린다).
 *   ⛔이 파일은 손대지 않는다 — 손대는 순간 「고치기 전」의 증거가 아니게 된다.
 *   report-queue-alias.test.mjs 의 H1-1(양성대조)이 이 파일을 그대로 require 해서, «지금»
 *   고친 main/report/queue.js 와 같은 시나리오를 돌려 «그때는 빨갰다»를 보인다.
 ══════════════════════════════════════════════════════════════════════════ */
/* ══════════════════════════════════════════════════════════════════════════
