 ...
 * ⛔그 보호는 «그날까지 참이었다». 2026-09-26 현빈 지시로 «옮겼다»:
 *     「여전히 빈칸으로 두고 싶은데 마지막 남은 줄은 삭제할 수 없다고 하네?」 ＋ 「②칸 하나만」
 *   까닭 전부는 js/blocks/grid-block.js `_gridRejectLinesLength` ＋ `_gridRejectAllCellsEmpty`.
 *   ★이 픽스처는 옆 칸(col1)에 기본 글줄이 있어 한 칸을 비워도 블럭이 안 빈다 ⇒ 통과한다.
 *     ⛔블럭의 «마지막 내용 칸»은 여전히 못 비운다(현빈 「②칸 하나만」) — 그 축은 grid-cell-emptied H 가 잰다.
 ...
 * ⛔그 보호는 «그날까지 참이었다». 0926 현빈 지시로 «옮겼다»(위 버그A-해제와 같은 까닭).
