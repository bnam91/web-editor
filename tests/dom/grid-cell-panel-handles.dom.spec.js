...
/* ★2026-09-25 «재촬영» — 59c6d63 → 9d00ec1 (「Grid (N×M)」 절 접기).
 *   까닭: 9d00ec1 이 패널 «맨 위» 피커 절을 접이식(기본 접힘)으로 바꿨다. 실측 —
 *     scrollHeight 1173(골든) → 910 · 줄바 아랫변 826 → 539 · 컨트롤 45 → 47.
 *   ⇒ Δ = −263px 이라 G2 ⑴ 의 ⛔「Δ<0 은 «절이 하나 안 뜬» 것일 수 있다」가 «옳게» 빨개졌다.
 *   ★그 의심에 답하는 것이 tests/dom/grid-panel-fold.dom.spec.js 다(같은 4×4 픽스처·같은 창).
 *     F2 가 접힘/펼침을 두 번 재어 ★DOM 수가 «같다»를 보인다 — 피커 칸 16 · 절 안 원소 수 ·
 *     안내문 수 · controlN · 절 수 · 절 제목 명부. 즉 «빠진» 것이 아니라 «접힌» 것이다.
 *     ⚠️controlN 만으로는 이 절을 못 잰다 — DOM 을 세는 것은 맞지만 이 절 «안»에는
 *       input/select/textarea/button 이 0 개다(피커 칸도 힌트도 div). 그래서 증인을 따로 세웠다.
 *   ⛔합격선 60 은 «한 글자도» 안 넓혔다. 기준점만 옮긴다 — 이 파일이 스스로 적어 둔 절차 그대로다.
 *   ⚠️★잃는 것을 적는다 (팀리드가 «알고» 받아야 하는 값):
 *     ⑴ 59c6d63 → ed74f65 사이에 이미 쌓여 있던 ★+20px 의 «빚»이 여기서 0 으로 리셋된다
 *        (ed74f65 의 제품코드로 재면 1193px · 컨트롤 47 이었다 — 골든 1173 · 45 대비 +20px/+2개).
 *     ⑵ ★그리고 이 재촬영은 «접힌» 910px 을 새 기준으로 박는다. 즉 앞으로 이 자가 재는
 *        「+60px 여유」는 ★피커 절이 «접혀 있는 상태»의 여유다. 누가 이 절을 다시 «기본 펼침»으로
 *        되돌리면 그 순간 +283px 이 되어 G2 가 빨개진다 — 그건 이 자의 «오작동»이 아니라
 ...
 * ⛔★이 픽스처에 `align`/`valign` 을 «하나도» 주지 마라 (2026-09-25 R3 실측).
 *   패널의 Layout 절은 「칸·줄에 따로 준 정렬은 지워진다 (⌘Z 복원)」를 «지울 게 있을 때만» 띄운다.
 *   여기에 정렬을 하나만 심어도 그 예고 한 줄이 줄바 «위»에 같이 떠서
 *     lineBarBottom 826 → 846 ⇒ 아래 G2 ⑵(「줄바가 더 밀리면 안 된다」 Δ≤0)가 빨개진다.
 *   지금 0건인 것은 «설계»가 아니라 «우연»이었다 — 그래서 그 전제를 기계로 못박아 뒀다:
 *     tests/dom/grid-panel-r1r6.dom.spec.js 의 R3-premise 가 이 블록을 소스로 읽어 «먼저» 빨개진다.
 *   ⇒ 정말 정렬 있는 픽스처로 재야 한다면 골든 «재촬영 절차»를 밟아라
 *     (합격선 60 무접촉 · 기준점만 · 제품코드 0줄 별도 커밋). */
const FIX_4x4 = (() => {
...
/* ★★이 자가 재는 «양» — 2026-09-24 실측으로 확정, 그리고 규약이 됐다.
 *   G2 의 「순증」은 «손잡이 수»가 아니라 ★«펼쳐진 높이»다.
 *   실측: 86dce84 이후 손잡이가 «열 개» 늘었는데 순증은 +38px 뿐이었다 — 열 개가 «전부»
 *   높이 0(「칸 꾸미기」·「줄 꾸미기」가 접이식 절이라)이었고, +38px 은 «절 머리 둘»의
 *   높이였다. 순진하게 폈으면 +240px 이다.
 *
 *   ⇒ ⛔★**손잡이를 접이식 절에 숨겨 이 자를 «0원»으로 지나가지 마라.**
 *     자주 쓰는 손잡이는 «펴서» 순증을 정직하게 내라. 60 을 넘으면 «재촬영 절차»를 밟아라
 *     — 합격선 60 무접촉 · 기준점만 · «제품코드 0줄 별도 커밋»(묶으면 재촬영이 증명을 덮는다).
 *     ※T-172 가 그 싼 길을 «안» 골랐다: 칸 테두리를 펼친 줄(h=24)로 두어 +35px 을 정직하게 냈고,
 *       그래서 이 자가 «제대로» 빨개져 재촬영 판단까지 갔다. 숨겼으면 아무 일도 안 났다.
 *
 *   ⚠️이건 이 자만의 병이 아니다 — ★치수·개수 게이트는 «언제나» 「숨기면 통과」를 공짜로 낸다.
 *     접힘 · display:none · 가상 스크롤 · 지연 렌더는 «전부 0»이다.
 *     ⇒ 그런 자를 새로 세울 땐 ★「무엇을 0 으로 세는가」를 «먼저» 적어라. */
