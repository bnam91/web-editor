...
/* ★1행 칸 — «cells» 에 진짜로 따로 저장되는 칸.
   ⛔0행 칸은 «열 그 자체»다(row 0 = cols[].lines). 그래서 0행 칸에 값을 주면 그 «열» 에 준 것이
     되고, 렌더러의 pick() 폴백(cell[k] ?? col[k]) 때문에 아래 행들까지 따라 칠해진다 — E10 참조.
...
/* ══════════════════════════════════════════════════════════════════════════
 * E10 — ★★«0행 칸»에 준 값은 그 «열 전체»로 샌다. (2026-09-23 이 그물이 찾아낸 것)
 *
 ...
 *
 * ★★2026-09-23 — 이 검사는 «빨간 것이 정상»으로 달았다(`test.fail`).
 *   ⇒ 이 실패는 T-178 이 «아직 살아 있다»는 표시다. 고쳐지면 여기가 «초록이 되어»
 *     「Expected to fail, but passed」로 실패한다. ★그때 이 검사도 같이 고쳐라
 *     (`test.fail` 을 `test` 로 되돌리고 단언의 방향을 뒤집는다). T-178 ⚠️ 에도 적혀 있다.
 *
 *   ⛔왜 «그냥 빨갛게» 두지 않았나 — 빨강 하나를 「카드가 안 닫혔다」는 표시로 쓰면
 ...
 *
 *   ⚠️`test.fail` 의 값: 「기대된 실패」는 «어떤 이유로든» 실패하면 초록이다 —
 *     그물이 통째로 망가져도(boot 실패·__mk 없음) 조용히 초록이 된다.
 *     ⇒ 그래서 «전제»를 아래 `E10-전제` 로 «따로» 뽑아 «늘 초록이어야 하는» 검사로 세웠다.
 *     그물이 죽으면 그쪽이 운다. 본 단언 둘만 여기 남긴다.
 * ════════════════════════════════════════════════════════════════════════ */
...
/* ★E10-전제 — «늘 초록이어야 하는» 짝. 아래 E10 은 `test.fail` 이라 어떤 이유로 죽어도
 *   초록이 된다. 그물이 망가졌는지는 «여기»가 알려 준다. ⛔이게 빨개지면 E10 의 초록은 헛것이다. */
...
test.fail('E10 ★«0행 칸»에 값을 주면 아래 행 칸까지 칠해진다 — 칸 손잡이를 만들기 «전»에 정해야 한다', async ({ page }) => {
