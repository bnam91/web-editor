 ...
 *     있나/없나: case "$(ps -o args= -p $P)" in *--user-data-dir=*) …   ← 아무것도 안 자른다
 *     값까지   : ps -o args= -p $P | sed 's/.*--user-data-dir=//; s/ --.*//'
 *   ⚠️값 쪽은 «휴리스틱»이라 완전하지 않다(실측): 경로에 공백이 있으면 `tr ' '` 판은 값을
 *     자르고, `sed 's/ --[a-z].*//'` 판은 뒤에 대문자 인자(--Foo)가 오면 그걸 값으로 삼킨다.
 *     위 판도 값 안에 " --" 가 들어가면 깨진다 ⇒ 값을 «판정 근거»로 쓸 땐 원본 인자도 같이 남겨라.
 *
