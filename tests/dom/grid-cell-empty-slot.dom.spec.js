 ...
 *   하셨다). 그래서 가드를 «옮겼다» — 까닭 전부는 js/blocks/grid-block.js
 *   `_gridRejectLinesLength` ＋ `_gridRejectAllCellsEmpty` 머리말, 새 계약의 그물은
 *   tests/dom/grid-cell-emptied.dom.spec.js. ⛔블럭의 «마지막 내용 칸»은 여전히 못 비운다(현빈 「②칸 하나만」).
 ...
 *   버그(칸이 주소를 잃음)의 부활이다」~~
 * ⛔옛 계약은 «그날까지 참이었다». 오늘 현빈 지시로 가드를 «옮겼으니» 그 단언은 «반드시» 빨개진다
 *   (이 픽스처는 옆 칸에 내용이 있어 새 범위 가드에 안 걸린다 — 그래서 «허용»이 보인다).
