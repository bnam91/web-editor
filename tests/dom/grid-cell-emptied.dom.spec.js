...
test('F ★내보내기 — 「+ 내용 추가」 안내문이 «안 샌다» ＋ ★양성대조 ＋ 죽은 min-height 기록', async ({ page }) => {
  ...
  /* ★★죽은 CSS 기록 — 위 머리말을 읽어라. 고쳤다면 이 줄이 빨개진다(그게 의도다).
     ~~[정정 · 2026-09-27] 「`0px` 이어야 한다」~~ ⇒ 이제 «14px» 다. 바뀐 것은 CSS 가 아니라
       «인라인»이다 — 렌더러가 빈 칸에 `min-height:14px` 를 박는다(GRID_EMPTY_CELL_MIN_H,
       현빈 0927 ㈏ 「틀이 보이게 나가게」). `#canvas … {48px}` 규칙은 «여전히 죽어 있다» —
       인라인이 이기기 때문이고, 그 죽음을 재는 것이 이 줄의 원래 뜻이다.
     ★48px 이 나오면 그때가 「CSS 가 살아났다」이고, T-229 결정(얇은 채로 둔다)이 뒤집힌 것이다. */
  expect(r.inCanvas.minH,
    '★빈 칸의 min-height 가 14px 가 아니다. 48px 이면 `#canvas … {min-height:48px}` 가 살아난 것이고 ' +
    '(T-229 결정이 뒤집힌다), 0px 이면 GRID_EMPTY_CELL_MIN_H 가 0 으로 돌아간 것이다(현빈 ㈏ 위반).')
    .toBe('14px');
...
test('I ★모든 칸이 비어도 «내보낸 결과물»에 자리가 남는다 (현빈 0927 ㈏)', async ({ page }) => {
  /* ~~[폐기 · 2026-09-27, 같은 날] 「모든 칸이 비면 내보내기 높이가 0 이다 — 고르고 남긴 계약」~~
     ⇒ 현빈이 ㈏(「틀이 보이게 나가게 — 와이어프레임을 그대로 내보내고 싶을 때」)를 고르셨다.
     ★고친 자리 — css/editor-blocks.css `.grid-block .grd-cell-empty { min-height: 14px }`
       (스코프 «없음». 바로 위 `#canvas … { min-height: 48px }` 와 일부러 다르다)
     ★이 검사가 재는 것 둘, 그리고 «갈라» 재는 까닭:
       ⑴ 내보내기에 자리가 남는가 — 현빈이 요구한 것
       ⑵ ★캔버스 높이가 «안 바뀌는가» — T-229 결정(「빈 칸 높이는 얇은 채로 둔다」)을 지킨다.
         ⛔⑵ 가 없으면 ⑴ 을 48px 로 «크게» 고쳐도 초록이 나고, 그건 다른 결정을 조용히 뒤집는다. */
  const errs = await boot(page);
  await page.evaluate(() => {
    document.getElementById('host').innerHTML = '';
