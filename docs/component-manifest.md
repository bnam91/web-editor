...
| card | `cvb_` | canvas-block(simple) | 이미지+제목+설명 반복 카드 (2026-06-11: 구 card-block/cdb 제거, canvas-block으로 통합) |
...
#### card (→ `cvb_` canvas-block simple mode)
- **쓸 때**: 이미지+제목+설명 묶음, 3-col 추천 제품 반복, 후기 카드, feature card.
- **쓰지 말 때**: 단일 hero → `canvas` layer mode. freeLayout → `canvas`.
- **예시 프롬프트**: "3-col grid에 추천 제품 카드 3개 (이미지 + 제목 + 가격)"
- **구현**: Spec `type:'card'`는 goditor-api가 `addCanvasBlock({cardMode:'simple', gridCols:count})`로 라우팅. 구 card-block(cdb)은 중앙패널 제거 + 타입목록 누락 버그로 2026-06-11 완전 삭제됨.
