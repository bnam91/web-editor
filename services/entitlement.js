 ...
 * ⚠️★이 값이 «라이브 서버의 그 키»인지는 아직 «교차검증 안 됐다»(2026-09-06).
 *   대성이 EC2 `/etc/goditor-api/env` 로 `publicKeyPem()` 을 1회 뽑아 대조해야 확정된다.
 *   그때까지 KEY_PROVENANCE 가 'unverified' 다 — E5 가 이 «한 줄»만 뒤집는다.
 */
...
const CONSTANTS = Object.freeze({
   ...
   *  = 릴리스일 + GRACE_DAYS. ★이 날이 지나면 단위검사 하나가 «실패»한다 —
   *    「릴리스 전에 날짜를 갱신하거나 2단계로 가라」를 경고가 아니라 검사로 강제한다. */
  SIGLESS_GRACE_UNTIL: '2026-10-21T00:00:00.000Z',
  ...
  VERIFY_BACKOFF_BASE_MS: 2000,     // 2s → 4s → 8s (지수)
