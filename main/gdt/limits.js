...
const LIMITS = {
  ...
  /* ★폰트 목록 (§11-2) — manifest.fonts 에는 상한이 «없었다».
   *   재현(2026-08-08, adversarial/fonts/font_flood.gdt): 5.6KB 짜리 .gdt 에 없는 글꼴 2,000개를
   *   실으면 「없는 글꼴」 대화상자가 셀렉트 2,001개(옵션 44만)를 그리며 9.3초를 멈췄고,
   *   그 목록이 localStorage 에 남아 목록 페이지를 열 때마다 되살아났다(지속형).
   *
   *   ⇒ 렌더러에서 «잘라 쓰는» 것으로도 막히지만 그건 «소비자» 방어다 —
   *     폰트 목록을 읽는 소비자가 하나 늘 때마다 다시 막아야 한다.
   *     남이 준 파일의 방어는 «경계»에서 한다. 그래서 여기에 둔다.
   *
   *   근거(실측 2026-08-08):
   *     실제 .gdt 5개      manifest.fonts 0~3개 · 이름 최장 14자
   *     실제 프로젝트 64개  고유 글꼴 최대 3종 · 이름 최장 18자
   */
  FONT_COUNT:           100,           // 정상 3 → 33배
  FONT_NAME_CHARS:      64,            // 정상 18자 → 3.5배.
                                       // ★렌더러 isSafeFamily 의 64자와 «같은 값»이어야 한다.
                                       //   두 곳에 다른 숫자가 있으면 「경계는 통과인데 화면엔 안 뜨는」 글꼴이 생긴다.
