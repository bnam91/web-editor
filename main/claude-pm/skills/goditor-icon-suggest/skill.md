...
- 위 매핑표로 prefix 1개 결정
- 무드가 애매하면 **prefix=`ph`** + **weight suffix=`-bold`** (Phosphor bold)를 기본값으로
  → API 호출 시 `search_iconify({prefix:'ph', query:'…'})`, 결과 fullName 중 `-bold`로 끝나는 것을 픽
  ⚠️ `'ph-bold'`를 prefix 그대로 넘기지 말 것 — 화이트리스트 fail (`mdi|material-symbols|heroicons|lucide|ph|tabler|bi|feather|ion|ri` 만 허용)
