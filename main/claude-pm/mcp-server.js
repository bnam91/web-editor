...
function _registerDefaultTools() {
  ...
  // ─── add_grid_block — 2~4열 × 1~4행 그리드 블록 추가 (P1, PLAN-gridblock.md §6) ──
  // duo-block.js makeDuoBlock 전체 opts 노출. blockId prefix 는 duo_(DOM 정체성은 P2까지 유지 —
  // 사용자에게는 "Grid"로 보인다). rows/cells 는 선택 — 생략하면 옛 duo 블록과 byte-identical 1행.
  registerTool(
    'add_grid_block',
    async (args = {}) => {
      if (!_rendererInvoker?.addGridBlock) throw new Error('renderer bridge not ready');
      const opts = _validateGridOpts(args, { mode: 'add' });
      return await _rendererInvoker.addGridBlock(opts);
    },
    {
      description: 'Add a grid block (2-4 columns x 1-4 rows; user-facing label "Grid", blockId prefix duo_). cols[2~4] required = [{width, align?, valign?, bg?, padding?, radius?, lines?}] — width is a relative weight (fr), not px. Each cell is a text stack: lines = [{type:label|h1|h2|h3|body|caption|image|gap, text?, fontSize?, color?, weight?, align?, marginTop?, imgSrc?, height?, radius?}]. rows[1~4] optional = [{height:"auto"|0-4000px}] — omit for a single-row grid (identical to the old duo block, no rows/cells written). cells optional = FULL rows x cols grid INCLUDING row 0 ([[{...cell},...],...]) — row 0 is folded into cols (single source of truth), only useful when rows.length>1. Returns {ok, blockId, sectionId, ...}. Edit with update_grid_block(blockId, partial).',
      inputSchema: {
        type: 'object',
        properties: {
          sectionId: { type: 'string', description: 'sec_xxx — omit to use currently selected section' },
          gap:       { type: 'integer', description: 'gap between cells px, both axes (0~200, default 24)' },
          valign:    { type: 'string', enum: ['top', 'middle', 'bottom'], description: 'block-level default vertical align (default top; a cell/col can override)' },
          cols: {
            type: 'array', minItems: 2, maxItems: 4,
            items: {
              type: 'object',
              properties: {
                width:   { type: 'number', description: 'relative weight (fr) — e.g. 1, 2, 0.5. Not px.' },
                align:   { type: 'string', enum: ['left', 'center', 'right'] },
                valign:  { type: 'string', enum: ['top', 'middle', 'bottom'] },
                bg:      { type: 'string', description: '#hex|rgb()/hsl()|transparent' },
                padding: { type: 'integer', description: '0~400' },
                radius:  { type: 'integer', description: '0~400' },
                lines: {
                  type: 'array',
                  description: 'row-0 content for this column (this column\'s cell in row 0)',
                  items: {
                    type: 'object',
                    properties: {
                      type: { type: 'string', enum: _GRID_LINE_TYPES },
                      text: { type: 'string', description: '≤2000 code points' },
                      fontSize: { type: 'integer', description: '4~400' },
                      color: { type: 'string' }, bg: { type: 'string' },
                      weight: { description: '100~900 | "normal" | "bold"' },
                      align: { type: 'string', enum: ['left', 'center', 'right'] },
                      marginTop: { type: 'integer', description: '0~400' },
                      imgSrc: { type: 'string', description: 'image type only — data:image/*, http(s)://, blob:, assets/ (≤200000)' },
                      height: { type: 'integer', description: 'image/gap type — px, 0~2000' },
                      radius: { type: 'integer', description: 'image type — px, 0~999' },
                    }
                  }
                }
              }
            },
            description: '2~4 columns — this IS row 0\'s content (single source of truth, see cells below)'
          },
          rows: {
            type: 'array', minItems: 1, maxItems: 4,
            items: { type: 'object', properties: { height: { description: '"auto" or a px number 0~4000 (minimum height — content can grow taller)' } } },
            description: 'optional — omit for a single row (byte-identical to the old duo block)'
          },
          cells: {
            type: 'array',
            items: { type: 'array', items: { type: 'object' } },
            description: 'optional — full rows x cols grid INCLUDING row 0 (row 0 folds into cols; rows 1+ need rows.length>1 to take effect)'
          }
        },
        required: ['cols']
      }
  ...
  // ─── update_grid_block — 그리드 블록 부분 수정 (id 기반, P1) ────────────────
  // 구조 필드(cols/patchCol/cells/patchCell)는 한 번에 하나만 — updateDuoBlock 검증과 동일 규칙.
  registerTool(
    'update_grid_block',
    async ({ blockId, ...rest } = {}) => {
      if (!_rendererInvoker?.updateGridBlock) throw new Error('renderer bridge not ready');
      if (typeof blockId !== 'string' || !blockId.startsWith('duo_')) {
        throw new Error(`invalid blockId: ${blockId}. must be a string starting with "duo_"`);
      }
      const partial = _validateGridOpts(rest, { mode: 'update' });
      if (Object.keys(partial).length === 0) {
        throw new Error('no fields to update — provide one of cols/patchCol/rows/cells/patchCell/gap/valign');
      }
      return await _rendererInvoker.updateGridBlock({ blockId, partial });
    },
    {
      description: 'Edit an EXISTING grid block (duo_xxx) — partial update. Exactly one of cols(2~4, full replace)|patchCol{index,...}(merge into one column)|rows(1~4, full replace)|cells(full rows x cols grid INCLUDING row 0, full replace)|patchCell{r,c,...}(merge into one cell, 0-based) may be given per call, plus gap/valign which combine freely. Row 0 is always the same data as cols (single source of truth) — patchCell{r:0,...} behaves exactly like patchCol. Returns USER_BUSY if the user is typing. Get blockId from get_canvas_state or add_grid_block.',
      inputSchema: {
        type: 'object',
        properties: {
          blockId:  { type: 'string', description: 'duo_xxx (grid block id)' },
          cols:     { type: 'array', minItems: 2, maxItems: 4, items: { type: 'object' }, description: 'full replace, 2~4 columns (see add_grid_block for shape)' },
          patchCol: { type: 'object', properties: { index: { type: 'integer', description: '0-based' } }, required: ['index'], description: 'merge fields into one column' },
          rows:     { type: 'array', minItems: 1, maxItems: 4, items: { type: 'object', properties: { height: {} } }, description: 'full replace, 1~4 rows = [{height:"auto"|px}]' },
          cells:    { type: 'array', items: { type: 'array', items: { type: 'object' } }, description: 'full replace, rows x cols grid INCLUDING row 0' },
          patchCell:{ type: 'object', properties: { r: { type: 'integer', description: '0-based' }, c: { type: 'integer', description: '0-based' } }, required: ['r', 'c'], description: 'merge fields into one cell' },
          gap:      { type: 'integer', description: '0~200' },
          valign:   { type: 'string', enum: ['top', 'middle', 'bottom'] }
        },
        required: ['blockId']
      }
...
// ─── grid(duo) 블록 검증 — P1 신설 ──────────────────────────────────────────
// cols[c] = 「행 0」 콘텐츠(js/blocks/duo-block.js getDuoGrid 와 같은 모델) + rows/cells/patchCell.
// ★단일 진실원 규칙을 여기서도 지킨다 — cells 는 항상 «행 0 포함 전체»를 받고(API 경계),
//   patchCol/patchCell{r:0}은 같은 셋(align/valign/bg/padding/radius/lines)을 merge 한다.
const _GRID_LINE_TYPES = ['label', 'h1', 'h2', 'h3', 'body', 'caption', 'image', 'gap', 'duo', 'graph'];
const _GRID_COLOR_RE = /^#[0-9a-fA-F]{3,8}$|^(rgb|rgba|hsl|hsla)\(\s*[\d.,\s%/]+\)$|^transparent$/;

function _gridColor(v, path) {
  if (typeof v !== 'string') throw new Error(`${path} must be string`);
  const s = v.trim();
  if (!s || s.length > 64 || !_GRID_COLOR_RE.test(s)) throw new Error(`${path} invalid color (allowed: #hex | rgb(a)/hsl(a)() | transparent)`);
  return s;
}
function _gridLine(l, path) {
  if (!l || typeof l !== 'object') throw new Error(`${path} must be object`);
  const o = {};
  if (l.type !== undefined) {
    if (typeof l.type !== 'string' || !_GRID_LINE_TYPES.includes(l.type)) {
      throw new Error(`${path}.type invalid: ${l.type}. allowed: ${_GRID_LINE_TYPES.join('|')}`);
    }
    o.type = l.type;
  }
  const _text = (k, max) => {
    if (l[k] === undefined) return;
    if (typeof l[k] !== 'string') throw new Error(`${path}.${k} must be string`);
    if ([...l[k]].length > max) throw new Error(`${path}.${k} too long (>${max} code points)`);
    o[k] = l[k];
  };
  _text('text', 2000);
  _text('content', 2000); // planner/generator 어휘 별칭 — 렌더러(duo-block.js _duoLineHtml)가 text 로 정규화한다
  if (l.fontSize !== undefined) {
    const n = Number(l.fontSize);
    if (!Number.isFinite(n) || n < 4 || n > 400) throw new Error(`${path}.fontSize must be 4~400`);
    o.fontSize = n;
  }
  if (l.color !== undefined) o.color = _gridColor(l.color, `${path}.color`);
  if (l.bg !== undefined) o.bg = _gridColor(l.bg, `${path}.bg`);
  if (l.weight !== undefined) o.weight = l.weight; // 100~900 | 'normal' | 'bold' 혼용 — 렌더러가 String() 캐스팅만 한다
  if (l.align !== undefined) {
    if (!['left', 'center', 'right'].includes(l.align)) throw new Error(`${path}.align must be left|center|right`);
    o.align = l.align;
  }
  if (l.marginTop !== undefined) {
    const n = Number(l.marginTop);
    if (!Number.isFinite(n) || n < 0 || n > 400) throw new Error(`${path}.marginTop must be 0~400`);
    o.marginTop = n;
  }
  if (l.imgSrc !== undefined) {
    if (typeof l.imgSrc !== 'string') throw new Error(`${path}.imgSrc must be string`);
    if (l.imgSrc.length > 200000) throw new Error(`${path}.imgSrc too long (>200000)`);
    if (/["\n\r]/.test(l.imgSrc)) throw new Error(`${path}.imgSrc: quotes/newlines not allowed`);
    o.imgSrc = l.imgSrc;
  }
  if (l.height !== undefined) {
    const n = Number(l.height);
    if (!Number.isFinite(n) || n < 0 || n > 2000) throw new Error(`${path}.height must be 0~2000`);
    o.height = n;
  }
  if (l.radius !== undefined) {
    const n = Number(l.radius);
    if (!Number.isFinite(n) || n < 0 || n > 999) throw new Error(`${path}.radius must be 0~999`);
    o.radius = n;
  }
  // 중첩 duo/graph 라인 — duo-block.js 렌더러가 자체 방어(depth 가드·slice(0,3)/(0,10))를 하므로
  // 여기선 구조(배열인지)만 얕게 확인해 통과시킨다(전체 재귀 스키마는 과설계 — P1 범위 밖).
  if (l.type === 'duo') {
    if (l.gap !== undefined) o.gap = l.gap;
    if (l.valign !== undefined) o.valign = l.valign;
    if (Array.isArray(l.cols)) o.cols = l.cols;
  }
  if (l.type === 'graph' && Array.isArray(l.items)) o.items = l.items;
  return o;
}
function _gridCol(c, path) {
  if (!c || typeof c !== 'object') throw new Error(`${path} must be object`);
  const o = {};
  if (c.width !== undefined) {
    const n = Number(c.width);
    if (!Number.isFinite(n) || n <= 0) throw new Error(`${path}.width must be a positive number (relative weight, not px)`);
    o.width = n;
  }
  if (c.align !== undefined) {
    if (!['left', 'center', 'right'].includes(c.align)) throw new Error(`${path}.align must be left|center|right`);
    o.align = c.align;
  }
  if (c.valign !== undefined) {
    if (!['top', 'middle', 'bottom'].includes(c.valign)) throw new Error(`${path}.valign must be top|middle|bottom`);
    o.valign = c.valign;
  }
  if (c.bg !== undefined) o.bg = _gridColor(c.bg, `${path}.bg`);
  if (c.padding !== undefined) {
    const n = Number(c.padding);
    if (!Number.isFinite(n) || n < 0 || n > 400) throw new Error(`${path}.padding must be 0~400`);
    o.padding = n;
  }
  if (c.radius !== undefined) {
    const n = Number(c.radius);
    if (!Number.isFinite(n) || n < 0 || n > 400) throw new Error(`${path}.radius must be 0~400`);
    o.radius = n;
  }
  if (c.lines !== undefined) {
    if (!Array.isArray(c.lines)) throw new Error(`${path}.lines must be array`);
    o.lines = c.lines.map((l, i) => _gridLine(l, `${path}.lines[${i}]`));
  }
  return o;
}
function _gridCols(arr, path) {
  if (!Array.isArray(arr)) throw new Error(`${path} must be array`);
  if (arr.length < 2 || arr.length > 4) throw new Error(`${path} must have 2~4 items`);
  return arr.map((c, i) => _gridCol(c, `${path}[${i}]`));
}
function _gridRowItem(r, path) {
  if (!r || typeof r !== 'object') throw new Error(`${path} must be object`);
  if (r.height === undefined || r.height === 'auto' || r.height === null || r.height === '') return { height: 'auto' };
  const n = Number(r.height);
  if (!Number.isFinite(n) || n < 0 || n > 4000) throw new Error(`${path}.height must be "auto" or a number 0~4000`);
  return { height: n };
}
function _gridRows(arr, path) {
  if (!Array.isArray(arr)) throw new Error(`${path} must be array`);
  if (arr.length < 1 || arr.length > 4) throw new Error(`${path} must have 1~4 items`);
  return arr.map((r, i) => _gridRowItem(r, `${path}[${i}]`));
}
function _gridCells(arr, path) {
  if (!Array.isArray(arr) || !arr.length) throw new Error(`${path} must be a non-empty 2D array (rows x cols, row 0 included)`);
  return arr.map((row, ri) => {
    if (!Array.isArray(row)) throw new Error(`${path}[${ri}] must be array`);
    // 셀은 col 과 같은 필드 집합(align/valign/bg/padding/radius/lines) — _gridCol 재사용.
    return row.map((cell, ci) => (cell == null ? {} : _gridCol(cell, `${path}[${ri}][${ci}]`)));
  });
}

function _validateGridOpts(args, { mode } = {}) {
  if (!args || typeof args !== 'object') throw new Error('args must be object');
  const out = {};
  ...
  if (mode === 'add') {
    ...
    if (args.cols === undefined || args.cols === null) throw new Error('cols required (array of 2~4 {width, lines?, ...})');
    out.cols = _gridCols(args.cols, 'cols');
    if (args.cells !== undefined && args.cells !== null) out.cells = _gridCells(args.cells, 'cells');
  } else {
    const structKeys = ['cols', 'patchCol', 'cells', 'patchCell'].filter(k => args[k] !== undefined && args[k] !== null);
    if (structKeys.length > 1) throw new Error(`${structKeys.join(', ')} 동시 지정 불가 — 구조 변경은 한 번에 하나만`);
    if (args.cols !== undefined && args.cols !== null) out.cols = _gridCols(args.cols, 'cols');
    if (args.patchCol !== undefined && args.patchCol !== null) {
      const p = args.patchCol;
      if (!p || typeof p !== 'object' || !Number.isFinite(Number(p.index))) throw new Error('patchCol must be {index, ...}');
      out.patchCol = { index: Number(p.index), ..._gridCol({ ...p, index: undefined }, 'patchCol') };
    }
    if (args.cells !== undefined && args.cells !== null) out.cells = _gridCells(args.cells, 'cells');
    if (args.patchCell !== undefined && args.patchCell !== null) {
      const p = args.patchCell;
      if (!p || typeof p !== 'object' || !Number.isFinite(Number(p.r)) || !Number.isFinite(Number(p.c))) {
        throw new Error('patchCell must be {r, c, ...}');
      }
      out.patchCell = { r: Number(p.r), c: Number(p.c), ..._gridCol({ ...p, r: undefined, c: undefined }, 'patchCell') };
    }
  }

  if (args.rows !== undefined && args.rows !== null) out.rows = _gridRows(args.rows, 'rows');
  if (args.gap !== undefined && args.gap !== null) {
    const n = Number(args.gap);
    if (!Number.isFinite(n) || n < 0 || n > 200) throw new Error('gap must be 0~200');
    out.gap = n;
  }
  if (args.valign !== undefined && args.valign !== null) {
    if (!['top', 'middle', 'bottom'].includes(args.valign)) throw new Error('valign must be top|middle|bottom');
    out.valign = args.valign;
  }
