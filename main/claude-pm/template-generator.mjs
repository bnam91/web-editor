...
export function renderClaudeMd(meta) {
  ...
  return [
    ...
    '- `add_asset_block` — 이미지 자리(비율 프리셋). `{ preset: "img1"|"img2"|"img3"|"text-img" }`. *이미지 생성 X, 비율 자리만*',
    '',
    '### 공통 규칙',
    ...
    '',
    '⚠️ 아직 없는 도구 (Phase 4+): `update_block`(기존 블록 수정) / `apply_theme`(컬러토큰) / 의도→구조 자동 플래너.',
    '   이런 동작 요청 시 "현재는 새로 추가만 가능, 기존 블록 수정/테마는 사용자 직접 또는 추후"라고 안내.',
    '',
