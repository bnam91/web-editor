...
export function renderClaudeMd(meta) {
  ...
  return [
    ...
    '## 🔴 무조건 규칙 — 스크래치→섹션 작업',
    '',
    '스크래치패드 아이템(sp_xxx)을 보고 캔버스 섹션을 **생성·수정·평가**하는 작업이면 → **반드시 `/goditor-layout-orchestrator` 스킬 사용**.',
    '',
    '- orchestrator는 maker(생성/수정) + evaluator(픽셀 비교) loop을 묶어 일치 도달까지 반복',
    '- read_scratch_item(JSON 데이터/dataURL prefix만)으로 "추측 비교" 금지 — 폰트 픽셀, 갭, 시각 무게 못 잡음',
    '- single evaluator만 호출도 금지 (fix loop 없으면 의미 없음)',
    '- 실제 캔버스 렌더 스크린샷 ↔ 스크래치 원본 픽셀 비교 결과로 update_block/update_section 호출',
    '',
