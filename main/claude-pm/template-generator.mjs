...
export function renderClaudeMd(meta) {
  ...
  return [
    ...
    '',
    '## Current Permission (Phase 2 MVP: 1개 write 도구 활성화)',
    '허용된 MCP tools:',
    '- `read_project` — 현재 활성 Goditor 프로젝트 JSON 전체 조회',
    '- `read_section` — sec_xxx 섹션 텍스트 추출',
    '- `list_memories` — NOTES.md / project.meta.json 메모 조회',
    '- `add_text_block` ⚡(Phase 2 MVP) — 캔버스 활성 섹션에 텍스트 블록 1개 추가',
    '   • params: `{ type: "body"|"h1"|"h2"|"h3" (default body), content: string (1~500자), sectionId?: string }`',
    '   • 결과: `{ ok:true, blockId, pageId }` 또는 `{ ok:false, code:"USER_BUSY", retryAfter }` (사용자 편집 중)',
    '   • 사용자가 입력 중일 때(activeElement editable / 최근 1.5s 키입력 / 자동저장 중)는 거절됨 → 잠시 후 재시도',
    '   • 동일 명령 무한 재시도 금지 — USER_BUSY 받으면 1회 재시도 후 사용자에게 양해 메시지',
    '',
    '⚠️ 그 외 캔버스 수정 (addSection / updateBlock / applyTheme) 은 Phase 3 예정.',
    '   사용자가 그 외 동작 요청 시 "현재 add_text_block만 가능. 의미 단위 섹션은 사용자 직접 또는 Phase 3"라고 안내.',
    '',
    ...
    '- `update_block(blockId, patch)` — 블록 텍스트/스타일 수정',
    '- `get_canvas_state()` — 현재 캔버스 트리 조회 (read_project 풍부 버전)',
    '- `apply_theme(blockId, token)` — Preference → 디자인 토큰 매핑',
    '- 의도→구조 매핑 플래너: "Hook 섹션 만들어줘" → 위 도구 시퀀스 자동',
    '- dry-run 미리보기 + 사용자 컨펌',
    '',
  ].join('\n');
}
