...
## 14. 정책 게이팅 (머지 직전 패치, 2026-06-25)
현빈 정책: "저장 시 자동 점진 외부화는 플래그(기본 OFF). 신규 이미지 자동 외부화는 유지. 기존 base64 대량변환은 명시 액션(optimizeProjectImages)만. 기존 프로젝트 로드는 무조건 비파괴."

### 메커니즘 — 로드 베이스라인(load baseline)으로 신규/기존 구분
- `applyProjectData()`(유일 로드 chokepoint) 에서 로드된 canvas의 모든 base64 data:URI 지문(fingerprint=length+말미24자)을 `_baselineByProject.get(projectId)` Set에 기록. **로드 자체는 아무 것도 변환하지 않음(비파괴).**
- 저장 시 `externalizeProjectData(data, projectId, {all})`:
  - `all:false` (기본 autosave) = **new-only** → 베이스라인에 **없는** base64(=이번 세션 신규)만 외부화. 기존 base64는 그대로 둠.
  - `all:true` (명시 최적화/레거시 플래그 ON) = 전부 외부화.
  - 베이스라인 미기록(레이스) → 안전하게 아무 것도 외부화 안 함.
- 플래그 `window.GOEDITOR_AUTO_EXTERNALIZE_LEGACY`(기본 false): true면 autosave도 레거시까지 점진 외부화(opt-in).
- `optimizeProjectImages()` = 명시 액션. `saveProjectToFile(snap, {externalizeAll:true})`로 전부 외부화 → 디스크 재로드 검증(섹션 수 보존 + base64 감소). 백업은 main `projects:save`가 자동(proj_backup.json + history slot).

### 만족
1) 플래그(기본 OFF) 게이팅 ✓  2) 신규 이미지 자동 외부화 유지 ✓(new-only 항상 동작)  3) 기존 대량변환은 optimize 버튼만 ✓  4) 기존 로드 비파괴 ✓(베이스라인 기록만).
