...
### 6-2. ★★외부화는 «용량 최적화»가 아니라 diff 의 «성립 조건»이다
> ⚠️ **이 절은 나중에 「외부화 왜 하지? 용량 때문이면 안 해도 되잖아」로 되돌리려는 사람을 위해 남긴다.**
> **용량을 포기해도 외부화는 포기할 수 없다.** 이유는 아래.
스냅샷은 정규형(`goya-asset://…`)인데 현재 proj.json 은 아직 base64 일 수 있다(미외부화 프로젝트).
그대로 비교하면 **이미지를 가진 모든 섹션이 「변경」으로 뜬다** — 목록이 통째로 쓸모없어진다.

**왜 이게 치명적인가.** 이 기능의 목적은 「사라진 섹션 찾기」다. 그런데 이미지가 든 섹션이 전부
「변경」으로 뜨면 사용자는 **어느 줄도 믿을 수 없다** — 목록이 노이즈로 덮여 도구가 통째로 무용해진다.
그리고 실사용 프로젝트의 대다수가 미외부화 상태다(히스토리 보유 61개 중 대부분). 즉 **이건 예외가 아니라
기본 케이스**다.

**즉 인과는 이렇다.** 외부화를 안 하면 → 스냅샷은 base64, 현재본도 base64 인데 «직렬화 시점이 달라»
바이트가 흔들리고, 스냅샷만 외부화하면 → 두 표현이 아예 다르다. **어느 쪽이든 비교가 성립하지 않는다.**
양쪽을 «같은 정규형»으로 몰아야만 비교가 가능하고, 그 정규형이 곧 외부화 형태다.
...
- ★**거부할 때는 «왜»를 화면에서 말한다** — 이유 없는 거부가 제일 나쁘다(지디 지시).
  타 인스턴스 판별 불가로 덮어쓰기를 막을 때:
  > 「다른 창에서 열려 있을 수 있어 교체할 수 없습니다 — **새 프로젝트로 복원**하세요」
  버튼도 그 자리에서 «새 프로젝트로»만 활성으로 남긴다(막고 끝내지 않는다).
...
### 8-0. ★★규약: «되돌릴 수단»을 대상과 «같은 봉투»에 두지 마라
현빈 질문에서 드러난 설계 결함이다 — **삭제 앞에서 이 기능 전체가 무력하다.**

`main.js` `projects:delete` 는 `proj_<id>/` 를 통째로 지운다(실측):
```js
fs.rmSync(dirPath, { recursive: true, force: true });   // 휴지통 미사용 (shell.trashItem 호출 0건)
```
그 봉투 안에 들어 있는 것 —
`proj.json` · `proj_backup.json` · **`proj_history/` 스냅샷 전부** · `assets/` · `proj_pre-externalize.json`.
⇒ **삭제 한 번에 본체·백업·히스토리·에셋이 «동시에» 사라진다.** 아무리 버전을 정교하게 쌓아도
   그 전부가 지워질 대상 «안»에 있으면 복구 수단이 아니다.
⇒ 별도로 `<id>_history/`(구 flat 잔재)도 같은 핸들러가 `rmSync` 한다 — 그건 §D3 폴백 체인이 읽는
   «복구 재료»인데 조용히 사라진다.

**규약**: 복구 수단은 대상 «밖»에 하나는 있어야 한다. P1 에서 그 역할은 **OS 휴지통**이 맡는다(§U7).
휴지통은 사용자가 비우면 끝이라 구조적 해법은 아니다 → 근본책은 P2(§12-C).
...
| **U7 삭제 안전망** ★신설 | `projects:delete` 의 `fs.rmSync` → `shell.trashItem`. P1 마지막 | U6b |

**의존 그래프**: `U0 → U1 → (U2 ∥ U4) → U3 → U5 → U6a → U6b → U7`
...
- ✅ **원본 `proj.json` 불변** — ★**대상 파일의 «바이트»를 직접 비교**한다(`fs.readFileSync` 두 벌의
  `Buffer.equals`). ⚠️ **디렉터리 md5/체크섬 롤업은 지표가 못 된다**(지디: 오늘 릴리스 라운드 교훈 —
  로드가 `st-protected-btn` 을 주입하는 것처럼 «무관한 것»이 섞여 들어와 신호가 죽는다).
  측정은 Electron 없는 node 하네스에서 — 앱이 돌면 autosave 가 원본을 정당하게 바꿔 «가짜 빨강»이 난다.
...
## §12-C. U7 — 삭제 안전망 (현빈 승인, P1 마지막)

### 무엇을 바꾸나
`main.js` `projects:delete` 의 `fs.rmSync(dirPath, …)` → **`shell.trashItem(dirPath)`**.
`shell` 은 이미 `main.js:1` 에서 임포트돼 있다(추가 의존성 0).

### 실측한 현행 (구현 전에 알아야 할 것)
| 사실 | 위치 |
|---|---|
| 번들 디렉터리를 통째 `rmSync` — 히스토리·에셋 포함 | `projects:delete` 본문 |
| flat 잔재 3파일 `unlinkSync` + `<id>_history/` `rmSync` | 같은 핸들러 후반 |
| 반환은 `dirOk` boolean (동기) | 핸들러 말미 |
| ⚠️**렌더러가 반환값을 «버린다»** — `await …deleteProject(id)` 결과 미확인 후 `renderGrid()` | `pages/projects.html:487~490` · `:646~655` |
| 확인 문구 = `"<이름>" 프로젝트를 삭제할까요?` (되돌림 가능성 언급 없음) | `pages/projects.html:651` |

★**렌더러가 결과를 버리는 게 U7 의 숨은 전제조건이다.** 지금은 삭제가 실패해도 화면에서 «성공»으로
보인다. 「실패하면 사용자에게 알린다」(아래 ⒜)를 채택하려면 **이걸 먼저 고쳐야 한다** — 안 고치면
정직한 실패 반환이 화면에 도달하지 않아 조치가 «없는 것과 같다».

### 결정
**D-U7-1 실패 시 «영구 삭제로 조용히 폴백하지 않는다»(지디 ⒜ 채택).**
복구 도구를 만들면서 「휴지통이 안 되니 영구 삭제할게요」는 앞뒤가 안 맞는다.
⇒ **다만 순수 ⒜는 사용자를 «지울 수 없는» 상태에 가둔다**(휴지통 없는 볼륨·꽉 참·권한).
   그래서 한 걸음 더: 실패를 그대로 알리되 **2차 확인으로 «영구 삭제»를 사용자가 «선택»하게** 한다.
   > 「휴지통으로 옮길 수 없습니다(<사유>). **영구 삭제**하면 되돌릴 수 없습니다. 진행할까요?」
   조용한 폴백(⒝)과 다른 점: **결정이 사용자에게 있고, 되돌릴 수 없다는 걸 알고 누른다.** 이게 정직이다.
   반환 계약: `{ ok, trashed:boolean, reason?:string }` — 「지웠다/못 지웠다」와 「휴지통이냐 영구냐」를 구분한다.

**D-U7-2 flat 잔재(`<id>.json` · `_meta` · `_backup` · `<id>_history/`)도 휴지통으로 보낸다.**
`<id>_history/` 는 §D3 폴백 체인이 읽는 «복구 재료»다 — 본체는 되살릴 수 있는데 그 옆의 복구 재료만
영구 소멸하면 규약(§8-0) 위반이다. 휴지통 항목이 여러 개로 늘지만, 잔재가 있는 프로젝트는 소수다.

**D-U7-3 ★휴지통에서 «찾을 수 있어야» 복구다.**
휴지통에 `proj_1787026440333` 같은 폴더가 수십 개 쌓이면 사용자는 자기 것을 못 고른다.
⇒ 휴지통으로 보내기 «직전»에 디렉터리 안에 `_deleted-info.json` 을 쓴다:
```json
{ "name": "세이프본 무릎보호대", "id": "proj_1787026440333", "deletedAt": "2026-08-28T…", "sections": 24 }
```
⛔**디렉터리 «이름»을 바꾸지 않는다** — id 가 곧 디렉터리명이라, rename 후 trash 가 실패하면
   살아 있는 프로젝트가 깨진 이름으로 남는다. 마커 파일은 어차피 버려질 봉투 안이라 위험이 0이다.

**D-U7-4 비동기 계약 + ★열려 있는 프로젝트 방어.**
`shell.trashItem` 은 Promise 다. 현행 `rmSync` 는 동기라 «지우는 순간»이 원자적이었는데,
await 구간이 생기면 **그 사이 autosave 가 `proj.json` 을 다시 쓸 수 있다**(반쯤 지워진 상태 = 최악).
⇒ 삭제 전에 «그 프로젝트가 열려 있나»를 렌더러가 답하게 하고, 열려 있으면 **탭을 먼저 닫는다**.
   닫을 수 없으면 삭제를 거부하고 이유를 말한다(§7-4 「거부할 땐 왜를 말한다」와 같은 규율).

**D-U7-5 화면 문구.**
`"<이름>" 프로젝트를 삭제할까요?` → `"<이름>" 을(를) 휴지통으로 보낼까요?\n\n휴지통에서 되살릴 수 있습니다.`
기대되는 되돌림 가능성이 달라졌으니 그걸 말해준다.

### Evaluator 인수기준 (지디 요구 + 보강)
- ✅ ★삭제 후 **`~/.Trash` 에 그 폴더가 «실제로» 있는지** 파일시스템으로 확인.
  「`trashItem` 이 에러를 안 냈다」는 증거가 아니다.
- ✅ ★**휴지통에서 되살린 뒤 목록에 다시 뜨고 열리는지** — 왕복 전체. 여기까지 돼야 「복구 가능」이다.
- ✅ `proj_history/` 스냅샷과 `assets/` 가 **같이 살아 돌아오는지**(봉투째라 당연해 보여도 «잰다»).
- ✅ ★**음성대조** — 휴지통 이동을 강제 실패시키면 **삭제가 «일어나지 않는지»**. 반쯤 지워지면 최악이다.
  (그리고 그때 2차 확인이 뜨고, 취소하면 프로젝트가 «멀쩡한지»)
- ✅ 확인창에서 «취소» → 아무 일도 안 일어나는지.
- ★보강 **렌더러가 실패를 화면에 반영하는지** — 위 「반환값을 버린다」가 안 고쳐졌으면 이 항은 자동 FAIL.
- ★보강 **flat 잔재도 휴지통에 갔는지**(영구 소멸 0).
- ★보강 **열린 프로젝트 삭제 시 반쯤 지워진 상태가 안 나오는지**(D-U7-4).
- ★보강 **되살린 프로젝트의 이미지가 보이는지** — `goya-asset://` 는 프로젝트 id 를 hostname 에 박고
  있으므로 같은 이름으로 복원돼야 한다. 「폴더는 돌아왔는데 그림이 빈다」가 제일 나쁜 결과다.

### ⚠️ 휴지통은 «구조적» 해법이 아니다 — P2 후보
사용자가 휴지통을 비우면 끝이고, 「비우기」는 우리가 통제할 수 없다.
⇒ **P2**: 삭제 «직전» 마지막 스냅샷을 프로젝트 «밖»(예: `<projectsDir>/_graveyard/<id>-<ts>.json`)에
   별도 보관 + 자체 보존기한. 그게 §8-0 규약을 구조적으로 지키는 유일한 방법이다.
   ⛔**P1 에선 안 한다**(범위 관리 — 현빈·지디 합의).
