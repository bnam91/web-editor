...
- **P-4 근사를 넣었으면 «최종 출력»까지 따라가라** (2026-08-28 신설 — 실데이터에 두 번 데이고 배웠다)
  근사·타협·플레이스홀더를 넣고 `approx: true` 같은 표시만 달아두는 건 **기억이지 검증이 아니다.**
  「그 근사가 화면에 무슨 문장으로 나오나」까지 가봐야 한다.
  ⚠️실제 사고: 페이지 경계를 못 가른 raw 지문에 `?::` 플레이스홀더를 넣고 `approx` 로 «표시»했는데,
  그게 손실 diff 를 지나 **「섹션 24개 전부 사라졌습니다」**가 되는 걸 못 봤다. 단위테스트 187개는
  전부 초록이었다 — 합성 픽스처는 «내가 아는 것»만 담기 때문이다.
...
### 6-3. 판정 규칙
- ★**손실 판정의 «신원»은 «섹션 id»다 — 페이지는 신원이 아니라 «위치»다.**(2026-08-28 실데이터로 정정)
  키는 `pageId::sectionId` 로 저장하되(표시·호환용), **비교는 `sectionId` 로 한다.** 두 가지 이유:
  ⒜ 섹션이 다른 페이지로 **옮겨간 것은 손실이 아니다** — 키 전체로 비교하면 살아 있는 섹션을 「사라졌다」고 말한다.
  ⒝ 대형 레거시 스냅샷은 JSON.parse 없이 raw 로 훑어 페이지 경계를 못 가른다(`fingerprintRaw` → `?::secId`).
     키 전체로 비교하면 **모든 섹션이 「사라졌다」**가 된다.
  ⚠️ **실측이 아니었으면 못 봤다** — 실프로젝트 60개 218버전 전수 스윕에서 **66건이 «전량 손실»**,
     그중 **최근 7일 버전은 27건 중 24건**이었다. 섹션 id 만 비교하니 교집합이 17/17·2/2 였다.
     즉 사고 직후 목록이 「24개 전부 사라졌습니다」라고 답하는 상태였다 — 복구 도구가 낼 수 있는 최악의 거짓말이다.
     수정 후 전량손실 8/218, 최근 7일 **0/27**. 합성 픽스처로는 절대 재현되지 않는 부류다.
  ⛔ 단 `noid_*` 폴백 키는 «위치»라 신원이 아니다 → 키 전체로 비교한다(조용히 「같음」으로 접으면
     사용자가 살릴 수 있었던 걸 못 살린다 — `history-diff.js` R6 과 같은 판단).
  ⛔ `changeDiff`(L2)는 이 규칙을 안 쓴다 — 거긴 «내용 비교»라 페이지 맥락이 유효하다.
- **옛 키 규약(저장 형식)** = `pageId::sectionId`. (`market-merge.js:41` 과 동일)
...
### ★구현하며 드러난 것 (적대검수 · 2026-08-28)
- **부분 이동이 제일 위험하다.** `trashItem` 을 여러 대상에 순차 호출하는데, k번째가 실패하면
  1..k-1 은 «이미 휴지통»이다. 초판은 그걸 「전부 실패」로 답해서, 렌더러가
  「휴지통으로 옮길 수 없습니다 → 영구 삭제할까요」라는 **사실과 다른** 확인창을 띄웠다.
  ⇒ `reason:'trash_partial'` + `deleted` 를 정직하게 돌려주고, 확인창이 「일부(N개)는 이미 옮겨졌습니다」를 말한다.
- ★**순서가 안전성을 가른다 — «잔재 먼저, 번들 나중»**. 반대로 하면 번들이 먼저 휴지통에 가고
  구 `<id>_history/` 가 남는데, 목록의 낡은 카드를 누르면 `projects:load` 폴백이 그 잔재로
  **옛 내용의 좀비를 되살린다**. 잔재 먼저면 최악이 「본체는 멀쩡, 낡은 잔재만 치움」이라 무해하다.
- **실패 경로도 화면을 다시 그려야 한다.** 초판은 취소·실패 `return` 이 `renderGrid()` 를 건너뛰어
  이미 휴지통에 간 프로젝트의 카드가 남았다 → 그 카드가 위 좀비의 입구였다.
- **ENOENT 는 «성공»이다**(연타·경합으로 먼저 치워진 것). 실패로 보면 이미 안전한 프로젝트에
  영구삭제를 권한다. 단 «우리가 옮긴 것»으로는 안 세야 사후조건과 충돌하지 않는다.
- **비동기 창의 사후조건**: 옮긴 뒤에도 디렉터리가 있으면 그건 「보냈다」가 사실이 아니다
  → `recreated_during_delete`. 잠금 대신 «결과 확인»으로 정직하게 답한다.
- 마커는 `permanent` 모드엔 안 쓰고, 실패 시 **살아남은 프로젝트에서 지운다**(거짓 `deletedAt` 방지).
  symlink 로 base 밖을 가리키는 경우를 `realpath` 로 한 번 더 막는다.

### ⚠️ 테스트로 «못 말하는 것» — 스텁 대 진짜 휴지통 (정직하게 남긴다)
하네스의 `shell.trashItem` 은 같은 볼륨 `renameSync` 다. 다음은 **이 테스트가 증명하지 못한다**:
- 진짜 휴지통은 외장/네트워크 볼륨·권한·휴지통 꽉 참에서 실패하고, 교차 볼륨이면 복사+삭제로
  «느리고 부분 복사»가 가능하다. 스텁의 rename 은 사실상 실패하지 않는다.
- 스텁은 동기적으로 끝나 **비동기 창이 구조적으로 안 생긴다** — 경합 계열은 손으로 주입해야만 잰다.
- 이름: 스텁은 `.타임스탬프` 를 붙이고 macOS 는 basename 을 보존하며 ` 2` 로 구분한다.
- ★**앱 안에 «되살리기» 경로가 없다** — 복구는 사용자가 Finder 에서 「제자리에 두기」를 해야 한다.
  `_deleted-info.json` 이 유일한 단서인데 **폴더 «안»에 있어 열어봐야 읽힌다.**
  ⇒ P2 후보: 앱 안 「최근 삭제」 목록(휴지통 스캔) 또는 §12-C 의 `_graveyard/`.
...
## §12-D. 이 브랜치가 «함께 고친» 별건 (추적용)

범위 밖이지만 **우리 코드가 같은 경로를 타서** 분리하는 게 오히려 부자연스러운 것만 여기서 고쳤다.
지디 승인 · 나중에 추적 가능하도록 여기에 남긴다.

### ① `_duplicateProjectImpl` 의 `collabRef` 상속 — 커밋 `9939d03`
| | |
|---|---|
| **증상** | 협업 등록 프로젝트를 «복제»하면 사본 `proj_meta.json` 이 원본의 `collabRef` 를 그대로 물려받는다. ⇒ **두 프로젝트가 같은 협업방을 가리켜 서로의 편집을 덮어쓴다.** |
| **범위** | 우리 U5(사본으로 열기)뿐 아니라 **기존 «복제» 버튼**(`pages/projects.html:529` ⎘)도 해당 — 즉 **이미 출시된 경로의 실버그**다. |
| **원인** | meta 복사가 `id`/`name`/`updatedAt` 만 덮고 나머지를 통째로 옮긴다. `collabRef` 는 `proj.json` 이 아니라 **meta 에만** 산다(`main.js:852` — 「원격 연결은 «문서»가 아니라 «이 설치»의 상태다」). |
| **수정** | 사본 meta 에서 `collabRef` 삭제 + `externalized` 마커 삭제(사본엔 `proj_pre-externalize.json` 이 없어 「변환 되돌리기」가 성립하지 않는다). |
| **왜 여기서** | U5 가 `_duplicateProjectImpl` 을 **재사용**한다(§8-2 — 복제 로직을 두 벌 만들지 않는다). 같은 함수라 분리 수정이 불가능하다. |
| **테스트** | `tests/unit/history-ipc.test.js` `U5-3` |

### ② 손대지 «않은» 것 — `js/market.js` 「머지 적용(새 프로젝트로 저장)」
`js/market.js:106~116` 이 `saveProject` 로 직접 프로젝트를 만들어 **에셋을 전혀 처리하지 않는다.**
`goya-asset://` 가 hostname 에 projectId 를 박으므로(`main.js:2237`) 그 새 프로젝트는 **원본 폴더를 몰래 참조**한다 — 원본 삭제 시 404.
⇒ **범위 밖이라 손대지 않았다**(지디 판단). 백로그 등록됨. 고칠 때는 `_duplicateProjectImpl` 경로로 옮기는 게 정답이다.
