...
> ---
> ## ⛔ 2026-09-05 — **A2 는 «철회»됐다 (현빈 결정, 지디 전달)**
> 현빈 사유: 「이 부분은 안 하기로 했다. **자동업데이트가 깨지기 때문**이다.」
> ⇒ **`build.win.artifactName` 을 제거해 원상복구했다**(`fix/r1-a2-revert`). 지금 `build` 설정은
> v0.9.0 릴리스 시점(`1bf10b2`)과 **버전 번호 말고는 한 글자도 다르지 않다.**
> **A1·A3 는 그대로 유효하며, A2 철회의 영향을 받지 않는다** — 근거는 §2-4.
> 아래 §2 와 §4/V3 는 «그때 무엇을 했고 무엇을 쟀나»의 기록으로 남긴다(지우지 않는다).
> ---
...
## 2. A2 — 「윈도우 산출물 파일명에 버전」  ⛔**철회됨(2026-09-05)** — 아래는 기록
...
### 2-4. ★A2 를 되돌려도 A1 검증은 무효가 되지 않는다 (2026-09-05 확인)
A1 의 파일명 폴백은 **pending 에 떨어진 파일 이름**을 읽는다. 그 이름은 `artifactName` 이 아니라
**`latest.yml` 의 `url` 을 그대로 따른다**(`AppUpdater.getCacheUpdateFileName()` = url pathname 의 basename).
- `artifactName` **있을 때**(내가 만든 피드): `GODITOR-Setup-0.9.1.exe`
- `artifactName` **없을 때**(현행·v0.9.0 정품): `GODITOR-Setup-0.9.0.exe` ← 세션 초 실측한 정품 `latest.yml` 의 `path`
**두 이름이 같은 형식**이다(공백→대시 치환이 같은 결과를 낸다). 그래서 폴백 파서가 읽는 것도 같다.
⇒ V5 의 실측(윈 527,055,774B → 0B)은 A2 철회 후에도 그대로 성립한다.
그리고 V5 의 **실제 케이스는 애초에 「정품 0.9.0 이 받은 파일」**이라 이미 A2 없는 세계의 이름이었다.
...
### V3 — A2 이름 3자 일치 : **PASS** ⛔**단 A2 자체가 철회돼 이 설정은 되돌렸다(2026-09-05)**
(mini05, github 설정 그대로 `--publish never`)
```
...
### 6-3. ⚠️ 사용자 지정 `artifactName` 은 arch 접미사를 «자동으로 안 붙인다»  *(A2 철회로 현재 무관 — 다음에 이 설정을 다시 건드릴 사람을 위해 남긴다)*
