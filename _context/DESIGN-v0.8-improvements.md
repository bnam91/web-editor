...
## 1-b. 게이트 확정값 (현빈 결정 2026-08-24 — 지디 전달)
- **#2** = AI 관련 2개(`#ai-btn.fp-btn-ai`·`#claude-pm-btn`)만 숨김 **종결**(커밋 fd7c0df). 나머지 9개 .fp-btn 현행 유지(숨김 아님).
- **#3** icon PNG = **외부화 경로(`assetsSaveCanvasImage`) 재사용**. data-URL 인라인 금지(proj.json 비대화 방지).
- **#8** 갭 병합 = **같은 section-inner의 «연속(인접)» 갭만** 합산. 떨어진 갭 병합 안 함.
- **#14** 회전 = **전 블록**(도형·텍스트·프레임·컴포넌트…). ★표·그래프 등 회전이 레이아웃/좌표에 문제되는 타입은 예외처리+상신. 회전후 리사이즈 핸들 좌표 보정 필수.
- **#1** 현행 유지(이름 변경 없음). **#16** 현빈 고민 중(다음 세션 대기).
