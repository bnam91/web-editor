...
|---|---|---|
| `js/history.js:115` | `restoreSnapshot` — 되돌리기 | 되돌리기는 «사람이 보고 있는 창»에서만 난다. 가려진 창에서 ⌘Z 가 오는 경로가 없다 ⇒ 급하지 않다 |
| `js/history.js:243` | `restoreSnapshotScoped` — 범위 되돌리기 | 위와 같다. 게다가 `window.scheduleAutoSave?.()` 를 같이 불러 재무장까지 한다 |
