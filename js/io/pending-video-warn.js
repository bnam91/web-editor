...
/** 래치를 처음 상태로 되돌린다.
 *
 * ★2026-09-22 검수로 «제품 코드도» 불러야 하는 것이 드러났다 — 앞 주석의 ⛔를 고친다.
 *   무엇이 있었나: ✕로 영상을 비운 뒤 «같은 파일»을 다시 넣으면 경고가 «0회»인데
 *   저장하면 영상이 실제로 사라진다.
 *   까닭 — 래치는 내용 신원(길이＋꼬리 32자)으로 걸리는데, ✕로 비우는 자리에서는
 *   warnPendingVideoLossIf 가 «안 불려» 래치가 안 풀린다. 같은 파일은 신원이 같으니
 *   「이 영상들로는 이미 알렸다」로 넘어간다.
 *   ★양성대조: 같은 자리에서 «다른» 영상을 넣으면 경고가 다시 뜬다 ⇒ 환경이 아니라 신원 충돌이다.
 *   ⛔이것이 조용한 데이터 소실이다 — 경고가 «안 뜨는 것»이 「안전하다」로 읽힌다.
 * ⇒ 미확정 영상을 «없애는» 자리에서 이 함수를 부른다(js/image-handling.js clearAssetImage).
