/* ═══════════════════════════════════
   BLOCK DRAG + FRAME DROP ZONE
   bindBlock (block selection, editing, drag events) +
   bindFrameDropZone + frame helpers
...
function bindBlock(block) {
    ...
    if (_parentFrameForMulti) {
      [..._parentFrameForMulti.children].forEach(ch => {
        ...
        const hasSelected = ch.querySelector(
          ...
          '.divider-block.selected, .bridge-block.selected,.duo-block.selected,.infocard-block.selected,.mockup-block.selected,.vector-block.selected,.step-block.selected'
        );
        if (hasSelected) {
          multiPeers.push({
            el: ch,
  ...
  if (isComparison) {
    block.addEventListener('click', e => {
      ...
      window.highlightBlock(block, block._layerItem);
      window.setBlockAnchor?.(block);
      window.showComparisonProperties?.(block);
    });
  }
  ...
  if (isBridge) {
    // 브릿지: divider와 동일한 클릭-선택 (정적 블록이라 applyDividerStyle 같은 재렌더 없음)
    block.addEventListener('click', e => {
      e.stopPropagation();
      const sec = block.closest('.section-block');
      if (e.metaKey || e.ctrlKey) { window.toggleBlockSelect?.(block, sec); return; }
      ...
      window.highlightBlock(block, block._layerItem);
      window.setBlockAnchor?.(block);
      window.showBridgeProperties?.(block);
    });
  }

  // duo/infocard: bridge와 동일한 클릭-선택 (dataset 모델 정적 블록)
  for (const [flag, showFn] of [[isDuo, 'showDuoProperties'], [isInfoCard, 'showInfoCardProperties']]) {
    if (!flag) continue;
    block.addEventListener('click', e => {
      e.stopPropagation();
      const sec = block.closest('.section-block');
  ...
  // section-inner.drop은 dragState.dragSrc(row 전체)를 이동시키며, .col에는 dragover/drop handler가 없음.
...
function bindFrameDropZone(ss) {
  ...
  ss.addEventListener('click', e => {
    ...
    if (e.target.closest('.text-block, .asset-block, .gap-block, .icon-circle-block, .table-block, .label-group-block, .graph-block, .divider-block, .bridge-block, .duo-block, .infocard-block, .icon-text-block, .joker-block, .shape-block, .canvas-block, .banner02-block, .comparison-block, .mockup-block, .vector-block, .step-block')) return;
  ...
  inner.addEventListener('drop', e => {
    ...
    const BLOCK_SEL = '.text-block, .asset-block, .gap-block, .icon-circle-block, .icon-block, .table-block, .label-group-block, .graph-block, .divider-block, .bridge-block, .duo-block, .infocard-block, .icon-text-block, .joker-block, .shape-block, .canvas-block, .banner02-block, .comparison-block, .mockup-block, .vector-block, .step-block';
    ...
    if (!isFreeLayout) {
      ...
      } else if (dragState.dragSrc.matches?.(BLOCK_SEL)) {
        // 단일 블록: row로 감싸서 삽입
        const existingRow = dragState.dragSrc.closest('.row');
      ...
      if (dragState.dragSrc.classList.contains('row') || (dragState.dragSrc.classList.contains('frame-block') && dragState.dragSrc.dataset.textFrame === 'true')) {
        if (dragState.dragSrc.dataset.textFrame === 'true') {
        ...
        const blocks = [...dragState.dragSrc.querySelectorAll(BLOCK_SEL)];
        ...
        dragState.dragSrc.remove();
        }
      } else {
        // dragSrc가 inner의 조상인 경우 삽입 금지 (HierarchyRequestError 방지)
        if (dragState.dragSrc.contains(inner)) { clearDropIndicators(); return; }
        const indicator = inner.querySelector('.drop-indicator');
        if (indicator) inner.insertBefore(dragState.dragSrc, indicator);
        else inner.appendChild(dragState.dragSrc);
