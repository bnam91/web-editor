 ...
 *   filter: drop-shadow 는 «칠해진 결과»(그라데이션으로 잘린 글자)의 알파로 그림자를 만들어 그 «뒤»에 깐다.
 *   ⚠️피그마 DROP_SHADOW 와 «같다»는 보장은 없다(미실측) — 피그마는 노드 모양 기준으로 알려져 있어, 알파가 0 인
 *   구간에서도 글자 윤곽 그림자가 남을 수 있다. 불투명 구간만 같은 모양일 것으로 «예상»한다.
 *
 * 변환 규칙
