...
/* ★연장 끝점 — «중심이 내 상자 안인 디바이스 행»까지만 (적대검수 조건④).
 * 앞판은 직선 구간을 «상자의 생 변»까지 늘렸다. 생 변이 디바이스 격자에 안 맞으면 그 좌표를 품은
 * 행이 경계를 «반씩» 걸치고, 그 행의 «중심»은 이웃 상자 안이다 — 그래서 A 만 선택했는데
 * 이웃 B 안이 4px 바뀌었다(실측: 공유 변 617.594, 행 y1235 중심 617.75 ∈ B, 18% 파랑 블렌드).
 *   ★내가 「y 범위가 상자와 같으니 §4-3 을 안 깬다」고 «논증»했던 자리다. 논증이 틀렸고 측정이 옳았다.
 * ⇒ 규칙을 «픽셀의 소유»로 다시 세운다: 행 i 는 [i/k,(i+1)/k) 이고 중심은 (i+0.5)/k.
 *   그 중심이 내 상자 «안»인 행까지만 칠한다.
 *   ⚠️그래도 «구멍 0» 이 유지된다 — 경계를 걸치는 그 행은 «중심이 있는 쪽» 상자가 가져가므로
 *     두 상자의 구간이 정확히 같은 좌표에서 만난다(맞물린다). 어느 쪽도 남의 픽셀을 안 건드린다.
 *   반경이 있는 모서리는 호가 대신하므로 이 연장을 쓰지 않는다(각진 모서리 전용). */
/* ★한 함수다 — 「위 상자가 끝나는 자리」와 「아래 상자가 시작하는 자리」가 «같은 좌표»여야
 *   구멍도 겹침도 안 생긴다. 경계 v 를 품은 행은 «중심이 있는 쪽» 하나만 가져간다.
 *   (처음엔 두 함수로 나눠 썼다가 v=100.25 처럼 «중심이 경계에 정확히 걸리는» 값에서
 *    두 상자가 같은 행을 «둘 다» 가져가는 것을 손계산으로 잡았다 — 그래서 하나로 합쳤다.) */
 ...
 * ★관측되는 모습 = «상한 바로 아래에서 진동»한다. 줌·위치에 따라 커지지도 작아지지도 «않는다».
 *   적대검수 독립 1,920 표본(줌 8종) max: 0.9781/0.9531/0.9125/0.9578/0.9844/0.9766/0.9922/0.9375
 *   — 어느 줌에서도 상한(h + 1/dpr)을 안 넘고, 줌과 «단조 관계가 없다». 표류가 아니라 상수 상한.
 * ⛔이 줄을 「줌이 올라갈수록 감소한다」로 쓰지 마라 — 그건 표본이 적을 때 보이던 착시였고
 *   2026-09-06 에 두 번(1,344 · 1,920 표본) 거짓으로 확인됐다. 커밋 40c5213 «메시지»에 옛 문장이
 *   남아 있으니 그것도 근거로 쓰지 마라(히스토리는 고칠 수 없어 여기에 대비를 세운다).
...
function _edgesOf(g) {
  const r = g.r || _ZERO_R;
  /* 반경 0 인 모퉁이에서는 직선 구간을 «중심이 내 상자 안인 마지막 디바이스 행»까지 늘린다(g.xlo…g.yhi).
   * ★반굵기 h 만 늘리면 구멍이 «절반만» 닫히고(실측 1 디바이스행 잔존),
   *   «생 변»까지 늘리면 구멍은 0 이지만 경계를 «걸친 행»을 칠해 이웃 안이 4px 바뀐다(조건④).
   *   ⇒ 「그 행을 «중심이 있는 쪽»이 가져간다」로 두면 «구멍 0 + 이웃 침범 0» 이 둘 다 선다. */
