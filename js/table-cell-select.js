...
(function () {
  ...
  // ★2026-09-04 P2: 구현은 drag-utils.js `suppressAncestorDrag`로 옮겼다(그리드 블록 열 경계
  //   드래그가 같은 문제를 겪어 공유화, PLAN-gridblock.md §5-B-4) — 동작은 완전히 동일(복붙 없음).
  //   ⚠️여기서 `const f = window.suppressAncestorDrag`로 «값을 미리 캡처»하면 안 된다 —
  //   이 파일은 classic script 라 drag-utils.js(ESM, deferred)보다 먼저 실행돼(index.html 순서)
  //   그 시점엔 아직 undefined 다. 실제 호출(mousedown 런타임)때 window 에서 다시 읽는다.
  function suppressAncestorDrag(block) { return window.suppressAncestorDrag(block); }
