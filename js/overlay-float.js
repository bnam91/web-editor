...
/* ★export — js/overlay-handles.js 의 오버레이 «크기조절» 핸들(T-068)이 이동과 «같은 곡선»을
   쓰도록 가져다 쓴다. 사본을 만들면 이동과 크기조절의 손맛이 조용히 갈라진다.
   (2026-09-20 통합 int/0920b: T-068 은 이 둘을 prop-text-wireup-overlay.js 에서 export 했는데,
    T-052 가 알맹이를 이 파일로 옮겼으므로 export 자리도 같이 옮긴다 — SSOT 는 한 곳이다.) */
...
export function exitFloat(posEl) {
  ...
  /* ★모서리 리사이즈가 «섹션 폭 밖까지» 키우려고 풀어 둔 maxWidth 를 되돌린다
     (overlay-handles.js _onTextOverlayResizeMouseDown — 현빈 2026-09-20 결정, T-068).
     ⛔_unfreezeWidth 의 overlayIntroducedWidth 갈래에 얹으면 안 된다 — 리사이즈는 그 도장을
       «떼고» 가므로(폭을 남겨야 하니까) 그 갈래로는 절대 안 들어온다. 도장을 따로 둔다.
