...
function createVariation(sec) {
  ...
  clone.querySelectorAll('.text-block, .asset-block, .gap-block, .icon-circle-block, .table-block, .label-group-block, .graph-block, .divider-block, .bridge-block, .duo-block, .infocard-block, .icon-text-block').forEach(b => window.bindBlock && window.bindBlock(b));
