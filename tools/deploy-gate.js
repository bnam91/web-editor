  ...
  try {
    ...
    if (!/^verified-\d{4}-\d{2}-\d{2}$/.test(oprov)) {
      process.stderr.write(
        ...
        '     ★배포 순서는 «4단계»다(어느 하나라도 빠지면 운영자가 잠긴다):\n' +
        '       ① 키쌍 생성 → ② 공개키를 services/operator-allow.js 의 OPERATOR_PUBLIC_KEYS 에 넣고\n' +
        '          PROVENANCE 를 verified-YYYY-MM-DD 로 «대조 표식» → 그 소스로 빌드\n' +
        '       ③ 운영자 PC 에 서명 operator.allow 설치 → ④ 그다음 배포.\n' +
        '       ②를 빼면 허가 파일을 깔아도 배포판은 «공개키 없음»이라 운영자를 전원 거부한다.\n' +
        '       순서가 거꾸로면 업데이트 순간 운영자의 PM·터미널·기획·라이선스 건너뛰기·MCP 로그인 인정이\n' +
        '       꺼집니다(옛 admin.allow·토큰은 무효).\n' +
        '     ⚠️이 경고가 사라지는 기준은 «키 개수»가 아니라 PROVENANCE 표식이다 — 표식만 고치고 키를\n' +
        '       안 넣으면 경고는 사라지지만 운영자는 그대로 꺼져 있다. 완료 신호로 쓰지 말 것.\n' +
        '     (경고만 — 배포는 계속됩니다)\n\n');
    }
  } catch (e) {
