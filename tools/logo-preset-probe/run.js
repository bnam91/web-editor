#!/usr/bin/env node
/* 로고 프리셋 유실 프로브 실행기 — 격리 프로필 헤드리스 크롬 + CDP(모든 호출 타임아웃).
...
const PORT = Number(process.env.PROBE_PORT || 9355);
const profile = mkTmpRoot('goya-logoprobe-');
let child;
...
(async () => {
  ...
  for (const c of R.cases) console.log(`  ${c.label.padEnd(26)} preset=${String(c.preset).padEnd(6)} ${c.w}×${c.h} ml=${c.ml || '-'}`);
