# Web Editor — Codex 작업 지침

> 이 파일은 프로젝트 전체에 적용되는 공통 원칙을 담는다.
> 특정 파일/모듈 전용 규칙은 해당 폴더의 AGENTS.md에 작성한다.
...
> CDP 디버깅 상세 규칙 (확인 방법, 허용 예외 등) → `debug/AGENTS.md` 참조
...
> **실수 예방**: 신규 블록 구현 후 QA 전에 위 5곳을 grep으로 교차 확인할 것. 특히 frame-block을 구조적으로 재사용하는 컴포넌트(`data-xxx-preset` 패턴)는 outline override를 잊기 쉬움 — `.Codex/rules/css-blocks.md` "컴포넌트 vs 컨테이너 outline" 섹션 필독.
...
| 경로 | 규칙 파일 | 내용 |
|------|---------|------|
| `css/**` | `.Codex/rules/css-blocks.md` | 블록 선택 스타일, speech-bubble outline, 코너 반경 핸들 |
| `js/**` | `.Codex/rules/js-structure.md` | 블록 구조 원칙, DOM 계층 구조 |
